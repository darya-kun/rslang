...
function PopUpMenu({togglePopMenu}) {
  ...
  const elements = items.map((item) => {
    return <div key={item.id}><Link to={item.link} style={{ textDecoration: 'none', color: 'grey' }}><PopUpMenuItem item={item} togglePopMenu={togglePopMenu}/></Link></div>
  });
         ...
         <div className="pop-up-menu">
           <span className="pop-up-menu__close-icon" onClick={() => togglePopMenu()}>
             <svg width="21" height="22" viewBox="0 0 21 22" fill="none" xmlns="http://www.w3.org/2000/svg">
               ...
               20.0863 21.1331 19.2263 20.6009 18.6945L12.4239 10.5172Z" fill="#2F281E"></path>
              </svg>
            </span>
